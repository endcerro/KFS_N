...
use crate::{dbg_println, m_println};
...
pub fn init() {
    ...
    dbg_println!("Tss structure initialized...");
    ...
    dbg_println!("GDT Segments initialized...");
    ...
    unsafe {
        memcpy(
            gdtr.address as *mut u8,
            segments.as_ptr() as *const u8,
            segments.len() * size_of::<SegmentDescriptor>() as usize,
        );
        dbg_println!("GDT copied to 0x{:x}, flushing..", GDTADDR);
        gdtflush(&gdtr);
        dbg_println!("GDT flush OK, TSS..");
        tssflush();
        dbg_println!("TSS flush OK");
    }
    #[cfg(feature = "gdt_test")]
    gdt_test_suite();
    dbg_println!("GDT load OK !");
}
...
unsafe fn tssflush() {
    core::arch::asm!("mov ax, 0x38", "ltr ax", options(nostack));
}
...
fn verify_gdt_load_structure() {
    ...
    memcpy(
        addr_of!(test_segments) as *mut _,
        gdtr.address as *const u8,
        gdtr.size as usize,
    );
    for i in 0..GDTSIZE {
        assert_eq!(correct_segments[i], test_segments[i]);
    }
    dbg_println!("GDT loaded and retrived successfully!");
}
...
pub fn verify_tss() {
    unsafe {
        ...
        assert_eq!(
            tss::TSS.esp0,
            addr_of!(stack_top) as u32 - KERNEL_VIRTUAL_BASE,
            "TSS ESP0 not set correctly"
        );
